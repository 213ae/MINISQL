...
BPlusTree::BPlusTree(index_id_t index_id, BufferPoolManager *buffer_pool_manager, const KeyProcessor &comparator,
                          int leaf_max_size, int internal_max_size)
        : index_id_(index_id),
          buffer_pool_manager_(buffer_pool_manager),
          processor_(comparator),
 ...
 */
bool BPlusTree::Insert(const GenericKey *key, const RowId &value, Transaction *transaction) {
  if(IsEmpty()){
 ...
 */
void BPlusTree::StartNewTree(const GenericKey *key, const RowId &value) {
 ...
 * keys return false, otherwise return true.
 */
bool BPlusTree::InsertIntoLeaf(const GenericKey *key, const RowId &value, Transaction *transaction) {
  auto leaf_page = reinterpret_cast<LeafPage *>(FindLeafPage(key));
 ...
 */
BPlusTreeInternalPage<GenericKey, page_id_t, KeyProcessor> *BPlusTree::Split(InternalPage *node, Transaction *transaction) {
...
BPlusTreeLeafPage<GenericKey, RowId, KeyProcessor> *BPlusTree::Split(LeafPage *node, Transaction *transaction) {
  page_id_t new_page_id;
 ...
 */
void BPlusTree::InsertIntoParent(BPlusTreePage *old_node, const GenericKey *key, BPlusTreePage *new_node,
...
void BPlusTree::Remove(const GenericKey *key, Transaction *transaction) {
  ...
  if(size_after_delete != -1){
    ...
    if(processor_(old_first_key, new_first_key) != 0 && !leaf_page->IsRootPage()){//检查删完后第一个pair是否发生了改变，如果改变，要更新父节点(如果不是root）
...
IndexIterator BPlusTree::Begin(const GenericKey *key) {
  ...
  if(index != -1) return IndexIterator(leaf_page->GetPageId(), buffer_pool_manager_, leaf_page->KeyIndex(key, processor_));
  else return End();
}
